...
public class TableOrderManager implements OrdersManager {
    ...
    public boolean addItem(Item item, int tableNumber){
    ...
    public void remove(Order order){
        ...
        for(int i=0;i<orders.length;i++){
            if(orders[i]!=null) {
                ...
                Item[] menuItems1 = orders[i].getItems();
                if(menuItems.length!=menuItems1.length) {
                    f=false;
                    break;
                }
                for(int j=0;j<menuItems.length;j++){
                    if (!(menuItems[j].getName().equals(menuItems1[j].getName()))||order.itemQuantity(menuItems[j].getName())!=orders[i].itemQuantity(menuItems1[j].getName())){
                        f=false;
                        break;
                    }
                }
    ...
    public int itemsQuantity(String itemName) {
        ...
        for (Order order : orders) {
            if (order != null) cost += order.itemQuantity(itemName);
        }
        return cost;
    }
    ...
    public int itemsQuantity(Item item) {
        ...
        for (Order order : orders) {
            if (order != null)
                cost += order.itemQuantity(item.getName());
        }
        return cost;
    }
