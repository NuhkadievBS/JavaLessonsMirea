...
public class InternetOrdersManager implements OrdersManager {
    ...
    public int itemsQuantity(String itemName) {
        ...
        for (Order order : orders) {
            cost += order.itemQuantity(itemName);
        }
        return cost;
    }
    ...
    public void remove(Order order) {
        ...
        do{
            ...
            Item[] menuItems1 = node.getValue().getItems();
            if(menuItems.length!=menuItems1.length) {
                f=false;
            }
            for(int j=0;j<menuItems.length;j++){
                if (!(menuItems[j].getName().equals(menuItems1[j].getName()))||order.itemQuantity(menuItems[j])!=node.getValue().itemQuantity(menuItems1[j])){
                    f=false;
                    break;
                }
            }
